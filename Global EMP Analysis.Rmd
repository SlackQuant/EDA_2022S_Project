...
FAA_strategy_ret <- eventReactive(input$go5, {
  ...
  FAA_basic <- FAAreturns(adPrices)
  bestN4 <- FAAreturns(adPrices, bestN=4)
  N5vol1cor1 <- FAAreturns(adPrices, weightVol = 1, weightCor = 1)
  ...
  all <- cbind(FAA_basic, bestN4, N5vol1cor1, minRisk, pureMomentum, maxDecor, momDecor)
  colnames(all) <- c("FAA_basic", "N4", "vol_1_cor_1", "minRisk", "pureMomentum", "maxDecor", "momDecor")
...
FAA_strategy_wt <- eventReactive(input$go5, {
  ...
  momDecor <- FAAweights(adPrices, weightMom=1, weightVol=0, weightCor=1)
