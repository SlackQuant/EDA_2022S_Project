...
AAA_strategy_wts <- eventReactive(input$go4, {
  ...
  lookback = 12                                 # 수익률을 측정할 과거 n 기간을 12개월로 설정
  wt_zero = rep(0, 11) %>%                      # rep() 함수를 통해 비중이 들어갈 0으로 이루어진 벡터를 만들고 이름을 설정
    setNames(colnames(rets))
  
  for (i in (lookback + 1) : length(ep)) {      # for loop 구문을 통해 매월 말 과거 12개월 수익률을 구한 후 비중을 계산하므로, 처음시작은 i+1인 13부터...  
    sub_ret = rets[ep[i - lookback] : ep[i], ]  # 과거 12개월 간 수익률을 찾은 후 sub_ret에 저장
    cum = Return.cumulative(sub_ret)            # 해당 기간의 자산별 누적수익률을 구함
    
    K = rank(-cum) <= 5                         # rank() 함수를 통해 수익률 상위 5개 자산을 선택, 내림차순으로 정렬해야 하므로 마이너스 붙여줌
    covmat = cov(sub_ret[, K])                  # cov() 함수를 통해 수익률 상위 5개 자산의 분산-공분산 행렬을 구하기
...
##### **<포트폴리오 배분>**
 ...
 1) 12개월 이내 모멘텀 스코어 기준 상위 5개 자산 선정

 2) 최소분산최적화 (Minimum Variance Optimization)를 통한 포트폴리오 구성
