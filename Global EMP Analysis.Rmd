...
ETF_rtn <- eventReactive(input$go, {     
  ...
  names(ETF_agg) <- c("Date", all_of(symbols))
...
AAA_strategy_ret <- eventReactive(input$go4, {

  # 1) 글로벌 10개 내외 자산 중 과거 12개월(또는 12개월 이내 평균수익률) 수익률이 높은 n개 자산을 선택
  # 2) 최소분산 포트폴리오를 구성, 개별 투자비중은 최소 10%, 최대 30% 등 적정 제약조건을 설정
  # 3) 월간 리밸런싱
  
  # Adaptive Asset Allocation : A Primer    SSRN 2328254
  # A Quantitative Approach to Tactical Asset Allocation
  
  ## Adaptive Asset Allocation : Momentum + min. variance
...
AAA_strategy_wts <- eventReactive(input$go4, {
  ...
  withProgress(message = 'In Progress', value = 0, {
    ...
    rets <- AAA_strategy_ret()
    
    # 백테스트에 사용되는 각종 값을 사전에 정의
    ep = endpoints(rets, on = 'months')           # 매월 말일의 위치를 구함
    wts = list()                                  # 매월의 투자비중이 들어갈 빈 리스트를 wts에 설정
    lookback = 12                                 # 수익률을 측정할 과거 n 기간
    wt_zero = rep(0, 12) %>%                      # rep() 함수를 통해 비중이 들어갈 0으로 이루어진 벡터를 만들고 이름을 설정
      setNames(colnames(rets))
    
    for (i in (lookback + 1) : length(ep)) {      # for loop 구문을 통해 매월 말 과거 12개월 수익률을 구한 후 비중을 계산하므로, 처음시작은 i+1인 13부터...  
      sub_ret_12 = rets[ep[i - lookback] : ep[i], ]         # 과거 12개월 간 수익률을 찾은 후 sub_ret에 저장
      cum_12 = Return.cumulative(sub_ret_12)                # 해당 기간의 자산별 누적수익률을 구함
      sub_ret_6 = rets[ep[i - lookback + 6] : ep[i], ]
      cum_6 = Return.cumulative(sub_ret_6)
      sub_ret_3 = rets[ep[i - lookback + 9] : ep[i], ]
      cum_3 = Return.cumulative(sub_ret_3)
      # sub_ret_1 = rets[ep[i - lookback + 12] : ep[i], ]
      # cum_1 = Return.cumulative(sub_ret_1)
      
      cum = (cum_12 + cum_6 + cum_3) / 3
      
      K = rank(-cum) <= 5                         # rank() 함수를 통해 수익률 상위 5개 자산을 선택, 내림차순으로 정렬해야 하므로 마이너스 붙여줌
      covmat = cov(sub_ret_12[, K])                  # cov() 함수를 통해 수익률 상위 5개 자산의 분산-공분산 행렬을 구하기
      
      # 임시로 비중이 저장될 wt변수에 위에서 만든 0벡터(wt_zero)를 입력한 후 optimalPortfolio()함수를 통해 최소분산 포트폴리오를 구성하는 해를 찾음
      # 개별 투자비중의 제한은 최소 10%, 최대 30%를 설정하며, 구해진 해를 wt의 K번째 값에 입력
      wt = wt_zero
      ...
      wts[[i]] = xts(t(wt), order.by = index(rets[ep[i]]))  # 위에서 만들어진 벡터를 xts()를 통해 시계열 형태로 바꾼 후 wts의 i번째 리스트에 저장
    }
    
    wts = do.call(rbind, wts)  # for loop 구문이 끝난 후 do.call() 함수를 통해 투자비중이 저장된 리스트를 테이블 형태로 바꿔줌
    
  })

})

FAA_strategy <- eventReactive(input$go5, {
  ...
  # data
  getSymbols(symbols, src = 'yahoo')  #, from = "2008-01-01", to = "2021-12-31")
  tmp <- list()
  for(fund in symbols) {
...
FAAreturns <- function(prices, monthLookback = 12,
                       weightMom = 1, weightVol = .5, weightCor = .5,
                       riskFreeName = "SHY", bestN = 5) {
  ...
  withProgress(message = 'In Progress', value = 0, {
    ...
    for(i in 1:(length(monthlyEps) - monthLookback)) {
    
      #subset data
      priceData <- prices[monthlyEps[i] : monthlyEps[i + monthLookback],]
      returnsData <- returns[monthlyEps[i] : (monthlyEps[i + monthLookback] - 1),]
    
      #perform computations
      momentum_12 <- data.frame(t(t(priceData[nrow(priceData),])/t(priceData[1,]) - 1))
      momentum_6 <- data.frame(t(t(priceData[nrow(priceData),])/t(priceData[nrow(priceData) / 2,]) - 1))
      momentum_3 <- data.frame(t(t(priceData[nrow(priceData),])/t(priceData[nrow(priceData) - (nrow(priceData) / 4),]) - 1))
      
      momentum <- (momentum_12 + momentum_6 + momentum_3) / 3
      ...
      vol <- as.numeric(-sd.annualized(returnsData[(nrow(returnsData) / 2) : nrow(returnsData)]))
      # sumCors <- -colSums(cor(priceData[endpoints(priceData, on="months")]))
      sumCors <- -colSums(cor(returnsData[(nrow(returnsData) / 2) : nrow(returnsData)], use = "na.or.complete"))
      ...
      } else if(nrow(stats) == 1 & rownames(stats) == "1") { #no securities had positive momentum
        longs <- 1
        names(longs) <- riskFreeName
      } else { #only one security had positive momentum 
        longs <- 1 / bestN
...
FAA_strategy_ret <- eventReactive(input$go5, {
  
  adPrices <- FAA_strategy()
  
  FAA_Opt <- FAAreturns(adPrices, weightMom = 1, weightVol = .5, weightCor = .5, bestN = 6)
  bestN3 <- FAAreturns(adPrices, bestN = 3)
  N6vol1cor1 <- FAAreturns(adPrices, weightMom = 1, weightVol = 1, weightCor = 1, bestN = 6)
  minRisk <- FAAreturns(adPrices, weightMom = 0, weightVol = 1, weightCor = 1, bestN = 6)
  pureMomentum <- FAAreturns(adPrices, weightMom = 1, weightVol = 0, weightCor = 0, bestN = 6)
  maxDecor <- FAAreturns(adPrices, weightMom = 0, weightVol = 0, weightCor = 1, bestN = 6)
  momDecor <- FAAreturns(adPrices, weightMom = 1, weightVol = 0, weightCor = 1, bestN = 6)

  all <- cbind(FAA_Opt, bestN3, N6vol1cor1, minRisk, pureMomentum, maxDecor, momDecor)
  colnames(all) <- c("FAA_Opt", "N3", "vol_1_cor_1", "minRisk", "pureMomentum", "maxDecor", "momDecor")
  
  return(all)
  
})

FAAweights <- function(prices, monthLookback = 12,
                       weightMom = 1, weightVol = .5, weightCor = .5,
                       riskFreeName = "SHY", bestN = 5) {
...
FAA_strategy_wt <- eventReactive(input$go5, {
  
  adPrices <- FAA_strategy()
  
  weight <- FAAweights(adPrices, weightMom = 1, weightVol = .5, weightCor = .5, bestN = 6)
...
### **Current Target Equity Weight (Global)**
...
##### **<데이터 출처 및 관련 자료>** 
 ...
 - **<[Using OECD Composite Leading Indicator Data to Time the Market](https://allocatesmartly.com/using-oecd-composite-leading-indicator-data-to-time-the-market/)>**
 
 - **<[Global Growth Cycle](https://grzegorz.link/growth)>**
 
 - **<[How good are early esimates of OECD Composite Leading Indicators?](https://www.oecd.org/sdd/leading-indicators/40182804.pdf)>**

### **Cumulative Performance : Korea  <[EWY](https://finance.yahoo.com/quote/EWY?p=EWY&.tsrc=fin-srch)>**
...
renderPlot({
  ...
  slice(-1) %>% 
  tk_xts(date_var = Date)
colnames(agg_tbl_xts) <- c("Strategy_KR", "Strategy_US", "Strategy_Global", "6:4_BM_KR", "6:4_BM_US", "6:4_BM_Global")
...
### **Cumulative Performance : USA <[SPY](https://finance.yahoo.com/quote/SPY?p=SPY&.tsrc=fin-srch)>**
...
### **Cumulative Performance : Global <[VT](https://finance.yahoo.com/quote/VT?p=VT&.tsrc=fin-srch)>**
...
renderTable({
  ...
  colnames(strategy_rtns_yr) <- c("Year", "Strategy_KR", "Strategy_US", "Strategy_Global", "6:4_BM_KR", "6:4_BM_US", "6:4_BM_Global")
...
actionButton("go4", "Download & Simulate")   
...
##### **<포트폴리오 배분>**
 ...
 1) 3, 6, 12개월 평균모멘텀 스코어 기준 상위 5개 자산 선정

 2) 최소분산최적화 (Minimum Variance Optimization : 12개월)를 통한 포트폴리오 구성

##### **<데이터 출처 및 관련 자료>** 
 - **<[Yahoo Finance](https://finance.yahoo.com/)>**
 
 - **<[Adaptive Asset Allocation](https://allocatesmartly.com/adam-butler-gestaltu-adaptive-asset-allocation/)>**
 
 - **<[Adaptive Asset Allocation: A Primer](https://papers.ssrn.com/sol3/papers.cfm?abstract_id=2328254)>**
 
 - **<[Adaptive Asset Allocation: Combining Momentum with Minimum Variance](https://cssanalytics.wordpress.com/2012/07/17/adaptive-asset-allocation-combining-momentum-with-minimum-variance/)>**
 
 - **<[Adaptive Asset Allocation – Systematic Investor](https://systematicinvestor.wordpress.com/2012/08/14/adaptive-asset-allocation/)>**
 
 - **<[Adaptive Asset Allocation – Sensitivity Analysis](https://systematicinvestor.wordpress.com/2012/08/21/adaptive-asset-allocation-sensitivity-analysis/)>**
...
renderPlot({
  ...
  rets <- AAA_strategy_ret()
  wts <- AAA_strategy_wts()
  
  AAA = Return.portfolio(rets, wts, verbose = TRUE)
  
  # turnover
  AAA$turnover = xts(
    rowSums(abs(AAA$BOP.Weight -
                  timeSeries::lag(AAA$EOP.Weight)),
            na.rm = TRUE),
    order.by = index(AAA$BOP.Weight))

  # 매매비용 등 반영
  fee = 0.002
  AAA$net = AAA$returns - AAA$turnover * fee
  
  ## 성과정리
  
  tbl_agg <- cbind(AAA$returns, AAA$net, rets$SPY, rets$EWY, rets$EEM) %>% 
    na.omit() %>% 
    setNames(c('Adaptive AA', 'Adaptive AA (after fee)', 'US', 'KR', 'EM'))
  
  tbl_agg %>% 
    charts.PerformanceSummary(main = 'Adaptive Asset Allocation')
...
renderTable({
  ...
  for (m in 1:5) {
    ...
    rtn_cum <- Return.cumulative(tbl_agg[, m]) # 누적수익률
    rtn_yr <- Return.annualized(tbl_agg[, m]) # 연율화 수익률(기하)
    
    std_yr <- StdDev.annualized(tbl_agg[, m]) # 연율화 변동성
    sr_yr <- SharpeRatio.annualized(tbl_agg[, m], Rf = risk_free, geometric = TRUE) # 기하평균 기준 연율화 수익률
    MDD <- maxDrawdown(tbl_agg[, m]) # 최대 낙폭
    cr_yr <- CalmarRatio(tbl_agg[, m])  # 연율화 수익률을 최대낙폭으로 나눈 값
    
    agg_temp <- rbind(rtn_cum, rtn_yr, std_yr, sr_yr, MDD, cr_yr)
    stack <- cbind(stack, agg_temp)
...
renderTable({
  ...
  stack2 <- NULL
  for (n in 1:5) {
    rtns_yr <- apply.yearly(tbl_agg[, n], Return.cumulative)
    stack2 <- cbind(stack2, rtns_yr)
  }
  
  # 티블로 바꿔주고 테이블 정리
  strategy_rtns_yr <- stack2 %>%
    tk_tbl(preserve_index = TRUE,
           rename_index = "Year") %>%
    as_tibble() %>% 
    setNames(c('Year', 'Adaptive AA', 'Adaptive AA (after fee)', 'US', 'KR', 'EM'))
...
renderTable({
  ...
  wts_tbl$Month <- format(wts_tbl$Month, "%Y-%m")
...
actionButton("go5", "Download & Simulate")   
...
##### **<포트폴리오 배분>**
 ...
 1) 3, 6, 12개월 평균모멘텀 스코어 기준 음(-)의 자산군 제거

 2) 모멘텀, 변동성, 상관관계 Composite Score 산출하여 상위 n개 자산 선정
 
 3) 3가지 지표에 대한 가중치 다르게 배분 가능

##### **<데이터 출처 및 관련 자료>** 
 - **<[Yahoo Finance](https://finance.yahoo.com/)>**
 
 - **<[Flexible Asset Allocation](https://papers.ssrn.com/sol3/papers.cfm?abstract_id=2193735)>**
 
 - **<[FAA : Alpha Architect](https://alphaarchitect.com/2013/01/flexible-asset-allocation/)>**
...
renderPlot({
  ...
  all <- FAA_strategy_ret()

  charts.PerformanceSummary(all, colorset=c("black", "red", "blue", "green", "darkgrey", "purple", "orange"), main = 'Flexible Asset Allocation')
...
renderTable({
  ...
  for (m in 1:7) {
    ...
    rtn_cum <- Return.cumulative(all[, m]) # 누적수익률
    rtn_yr <- Return.annualized(all[, m]) # 연율화 수익률(기하)
    
    std_yr <- StdDev.annualized(all[, m]) # 연율화 변동성
    sr_yr <- SharpeRatio.annualized(all[, m], Rf = risk_free, geometric = TRUE) # 기하평균 기준 연율화 수익률
    MDD <- maxDrawdown(all[, m]) # 최대 낙폭
    cr_yr <- CalmarRatio(all[, m])  # 연율화 수익률을 최대낙폭으로 나눈 값
...
renderTable({
  ...
  stack2 <- NULL
  for (n in 1:7) {
    rtns_yr <- apply.yearly(all[, n], Return.cumulative)
    stack2 <- cbind(stack2, rtns_yr)
  }
...
### **FAA_Opt Weights History (%)**

```{r}
  
renderTable({
  
  weight <- FAA_strategy_wt()
